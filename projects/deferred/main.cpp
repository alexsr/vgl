...
struct G_buffer {
    ...
    void init(glm::ivec2 size) {
        ...
        vgl::gl::attach_draw_buffers(fbo, { GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1, GL_COLOR_ATTACHMENT2, GL_COLOR_ATTACHMENT3 });
...
int main() {
    ...
    vgl::gl::glprogram deferred_prepass, deferred_lighting, depth_prepass, lights_debug,
        aabb_debug, screen, cubemap, cubemap_equirect, frustum_culling;

    auto reload_shaders = [&]() {
        auto frustum_culling_source = glsp::preprocess_file((vgl::file::shaders_path / "util/frustum_culling.comp").string()).contents;
        ...
        auto deferred_prepass_vs_source = glsp::preprocess_file((vgl::file::shaders_path / "shading/shading.vert").string()).contents;
        auto deferred_prepass_fs_source = glsp::preprocess_file((vgl::file::shaders_path / "shading/deferred_geometry.frag").string()).contents;
        ...
        auto deferred_lighting_fs_source = glsp::preprocess_file((vgl::file::shaders_path / "shading/deferred_lighting.frag").string()).contents;
    ...
    while (!window.should_close()) {
        ...
        if (ImGui::Begin("Settings")) {
            if (ImGui::Checkbox("View frustum culling", &config.activate_frustum_culling)) {
                if (config.mesh_loaded && !config.activate_frustum_culling) {
                    ...
                    glDispatchCompute(glm::ceil(scene.objects.size() / static_cast<float>(wgs.at(0))), 1, 1);
        ...
        if (config.mesh_loaded) {
            ...
            if (config.activate_frustum_culling) {
                ...
                std::array<int, 3> wgs;
                glGetProgramiv(frustum_culling, GL_COMPUTE_WORK_GROUP_SIZE, wgs.data());
                glDispatchCompute(1 + glm::ceil(scene.objects.size() / static_cast<float>(wgs.at(0))), 1, 1);
            }
